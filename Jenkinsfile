    ...
    stages{
        ...
        stage('Build Docker Images') {
            steps {
                ...
                sh '''
                docker build --no-cache -f ./app/Dockerfile -t ${DOCKERHUB_ID}/${env.ICWEBAPP_IMAGE} ./app
                '''
            }
        }
        ...
        stage('Scan Image with  SNYK') {
            ...
            steps {
                script{
                    sh '''
                        echo "Starting Image scan ${DOCKERHUB_ID}/${env.ICWEBAPP_IMAGE} ..."
            ...
            steps {
                echo "Lancement des conteneurs pour tests"
                    sh '''
                        ...
                        docker run -d --name test_icwebapp -p 8090:8080 ${DOCKERHUB_ID}/${env.ICWEBAPP_IMAGE}
